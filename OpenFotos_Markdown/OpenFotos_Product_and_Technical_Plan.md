...
10. Owner and guest URLs are independent high-entropy capabilities. A PIN never provides
    security on its own.
...
12. Selfie/reference search accepts exactly one usable face and returns every match above
...
17. Owner and guest capabilities last at most 365 days, use one-time fragment-secret/PIN delivery,
    and create 12-hour and 24-hour sessions respectively. Owner revocation cascades to all guests.
...
| Owner link | Photographer-to-main-customer capability for event browsing and guest management. |
| Guest link | Owner-created capability with an event or immutable sub-event scope. |
...
9. Issues the owner link, saves its one-time credential card, and hands it to the main customer.
10. Can inspect/revoke guest links and download visible exact originals.
...
- Archived sub-events remain recorded for audit and restoration but are absent from event snapshots,
  gallery queries, upload targets, and new share creation.
...
### 6.4 Owner and guest access

Session 8 removes the interim event token/PIN fields. A photographer issues one event owner
capability; the owner creates independently revocable whole-event or sub-event guest capabilities.
...
- Publication requires a current committed ingestion manifest, confirmed preview policy, complete
  derivatives for every visible asset, at least one active sub-event, and a future expiry.
...
- Owner/guest link policy, PIN verification, search scope, and non-authorizing result state.
...
OwnerCapability/GuestCapability 1---* FaceSearchResultSet
```

Capability secrets are stored as digests and PINs as peppered Argon2 hashes. Raw selfies and face
crops are not stored. One-hour search-result state contains only ordered asset IDs and grants no
photo access.
...
- JPEG, PNG, WebP, HEIC, and HEIF are accepted up to 20 MiB and 40 megapixels. Every attempt
  requires an explicit permission/consent checkbox.
...
- Ten attempts per browser/link pair and 100 total attempts per capability are allowed in each
  15-minute window.
- Pagination persists only ordered matched asset IDs for one hour. Clear/revocation removes them
  immediately, and an idempotent cleanup command removes expired rows.

Session 7 implements the exact event/sub-event PostgreSQL query boundary. Session 8 connects it to
the public owner/guest reference-image endpoint and UI without making the result an authorization
record.

## 14. Sharing (Session 8 implemented)
...
- Allows All Photos, all active sub-events, scoped face search, original downloads, and guest-link
  management.
- Lasts at most 365 days, capped by event expiry, and creates a session lasting at most 12 hours.
- Revocation/reissue invalidates every derived guest and outstanding search-result record.
...
- optional private label up to 80 characters, omitted from audit metadata and logs.

Every guest can browse, search, and download all visible photos in scope. Face search filters that
set and does not authorize photos. A guest link cannot create another link or widen itself. No more
than 100 active guest links may derive from one owner, and a guest session lasts at most 24 hours.
...
- Argon2 PIN hashing with a dedicated server-side `SHARE_PIN_PEPPER`;
...
The URL secret is carried in the browser fragment, removed from history, and exchanged for an
HttpOnly presentation cookie so it is absent from HTTP request targets and referrers. The link and
PIN appear once on a no-store copy/download card. `SHARE_PIN_PEPPER` has no periodic rotation; a
suspected compromise requires the explicit all-links reset before replacement.
...
original upload checksum. Downloads are individual exact originals rather than ZIPs and may retain
source EXIF/GPS metadata; the photographer workflow warns about this property.
...
- Minimize data: retain one search index only as long as the event needs search; never retain search
  selfies, crops, or query vectors; retain ordered result IDs for at most one hour; avoid filenames
  and media URLs in logs.
...
- Face endpoints use the accepted Session 6 model contract. Owner/guest endpoints bind every
  session to capability, event, access-version, and capability kind.
...
- Audit events cover authentication, PIN outcomes, event transitions, installation registration and
  revocation, sub-event changes, batch reassignment, exclusions, publication, capability changes,
  face-search outcomes, and original-download issuance.
...
- Session 8 end-to-end tests prove reference-input cleanup, capability-scope enforcement, and exact
  original signing.
...
Complete. Owner and independently revocable full-gallery guest capabilities use fragment secrets,
four-digit generated PINs, immutable event/sub-event scope, capped expiry, short sessions,
throttling, and audit. Exactly-one-face search is an ephemeral, consented gallery filter whose
...
- publication blocks until every visible current-generation asset is `indexed` or
  `no_usable_face`;
- owner/guest secrets and PINs are shown once, never stored raw, and revocation/expiry/unpublication
  invalidate residual sessions without widening scope;
- a sub-event guest cannot browse, search, or download a sibling photo, even when its vector is the
  closest match;
